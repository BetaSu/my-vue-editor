...
const t = {
  'itodo' (rh, data) {
    ...
    commands['insertHTML'](rh, `<${constant.ROW_TAG} data-editor-todo=${todoId} contenteditable="false"><label class="iui-icon iui-icon-checkbox" for="${todoId}"></label><input id="${todoId}" type="checkbox"/><input type="text" placeholder="${data.placeholder}"></${constant.ROW_TAG}><br>`)
  ...
  'initiTodo' (rh, data) {
    ...
    checkboxs.forEach((c, index) => {
      ...
      let label = c.querySelector('label')
      if (c.init) return
      ctnCheckedLogic()
      ...
      function ctnCheckedLogic() {
        ...
        if (btn.checked) {
          ...
          label.className = 'iui-icon'
          label.classList.add('iui-icon-checked')
        } else {
          ctn.style.textDecoration = 'none'
          btn.removeAttribute('checked')
          label.className = 'iui-icon'
          label.classList.add('iui-icon-checkbox')
        }
